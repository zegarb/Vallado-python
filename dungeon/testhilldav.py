...
for ktr in range(1, 11):
    ...
    rhillx, vhillx = ECI_to_EQCM_RTN_sal(rtgteci, vtgteci, rintecix,
                                         vintecix, outfilehill)
    #        [rintecix, vintecix]   = eqcm2eci( rtgteci, vtgteci, hrokm, hvokm );     # find int pos eqcm
    #        [rhillx, vhillx]      = eci2eqcm( rtgteci, vtgteci, rintecix, vintecix ); # find how much this would be for true hills
    dr = rhillx - hrokm
    dv = vhillx - hvokm
...
if (juststk != 0):
    ...
    for ktr in range(1, 7):
        ...
        print('hillsin       %20.13f %20.13f %20.13f %20.13f %20.13f %20.13f \n' % (hrokm[0] * 1000, hrokm[1] * 1000, hrokm[2] * 1000, hvokm[0] * 1000, hvokm[1] * 1000, hvokm[2] * 1000))
        ...
        mmagri = smu.mag(rintecisal)
        mmagvi = smu.mag(vintecisal)
        print('rinteci        %20.13f %20.13f %20.13f   %20.13f %20.13f %20.13f  %15.8f %15.8f \n' % (rintecisal[0], rintecisal[1], rintecisal[2], vintecisal[0], vintecisal[1], vintecisal[2], mmagri, mmagvi))
    ...
    for ktr in range(1, 7):
        ...
        rintecisal, vintecisal = EQCM_to_ECI_RTN_sal(rtgteci, vtgteci, hrokm,
                                                     hvokm)
        ...
        rhillsal1, vhillsal1 = ECI_to_EQCM_RTN_sal(rtgteci, vtgteci,
                                                   rintecisal, vintecisal,
                                                   outfilehill)
        ...
        if (ktr <= 8):
            ...
            mmagrti = smu.mag(rintecisal)
            mmagvti = smu.mag(vintecisal)
            print('rintecisal hil %20.13f %20.13f %20.13f %15.8f %20.13f '
                  '%20.13f %20.13f %15.8f \n'
                  % (rintecisal[0], rintecisal[1], rintecisal[2], mmagrti,
                     vintecisal[0], vintecisal[1], vintecisal[2], mmagvti))
            mmagrti = smu.mag(rintecisal1)
            mmagvti = smu.mag(vintecisal1)
            print('rintecisal1 eq %20.13f %20.13f %20.13f %15.8f %20.13f '
                  '%20.13f %20.13f %15.8f \n'
                  % (rintecisal1[0], rintecisal1[1], rintecisal1[2], mmagrti,
                     vintecisal1[0], vintecisal1[1], vintecisal1[2], mmagvti))
            print('hillssal hill  %20.13f %20.13f %20.13f %20.13f %20.13f '
                  '%20.13f \n'
                  % (rhillsal[0] * 1000, rhillsal[1] * 1000,
                     rhillsal[2] * 1000, vhillsal[0] * 1000,
                     vhillsal[1] * 1000, vhillsal[2] * 1000))
            print('hillssal1 eq   %20.13f %20.13f %20.13f %20.13f %20.13f '
                  '%20.13f \n'
                  % (rhillsal1[0] * 1000, rhillsal1[1] * 1000,
                     rhillsal1[2] * 1000, vhillsal1[0] * 1000,
                     vhillsal1[1] * 1000, vhillsal1[2] * 1000))
            print('==== sals codes ====\n' % ())
            ...
            print('rintecisal hil %20.13f %20.13f %20.13f %15.8f %20.13f '
                  ...
                  % (rintecisal[0] * 0.001, rintecisal[1] * 0.001,
                     rintecisal[2] * 0.001, mmagrti * 0.001,
                     vintecisal[0] * 0.001, vintecisal[1] * 0.001,
                     vintecisal[2] * 0.001, mmagvti * 0.001))
            print('rhills2 in     %20.13f %20.13f %20.13f %20.13f %20.13f '
...
rad = 180.0 / np.pi
rtgteci = np.array([0.0, 9380.502297, 5093.197188]).T
vtgteci = np.array([- 6.110899, 3.222217, 1.749521]).T
...
hrokm = np.array([79.37875, 0.0,- 93.843398]).T
hvokm = np.array([- 0.038118,- 0.068157,- 0.031665]).T
...
#stk ans
rintans = np.array([1e-12, 9495.039900814565, 5048.602269265176]).T
vintans = np.array([- 6.088186588973, 3.203827617391, 1.703505361584]).T
print('STK answer            %20.13f  %20.13f  %20.13f  %20.13f  %20.13f  %20.13f \n' % (rintans[0], rintans[1], rintans[2], vintans[0], vintans[1], vintans[2]))
...
print('rinteci EQCM ntw     %20.13f  %20.13f  %20.13f  %20.13f  %20.13f  %20.13f \n' % (rinteciz[0], rinteciz[1], rinteciz[2], vinteciz[0], vinteciz[1], vinteciz[2]))
