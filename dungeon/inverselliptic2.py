...
def inverselliptic2(E = None,m = None,tol = None):
    if len(varargin) < 3:
        tol = eps
    ...
    if len(m) == 1:
        m = m(np.ones((E.shape,E.shape)))

    if len(E) == 1:
        E = E(np.ones((m.shape,m.shape)))

    if not m.shape==E.shape :
        raise Exception('E and M must be the same size.')

    invE = np.zeros((E.shape,E.shape))
    # make a row vector
    m = m
    E = E
    ...
    __,E1 = ellipke(m,tol)
    zeta = 1 - z / E1
    r = np.sqrt(np.multiply(zeta,zeta) + np.multiply(mu,mu))
    theta = np.arctan(mu / (z + eps))
    # Empirical initialization [1]
    invE = np.pi / 2 + np.multiply(np.sqrt(r),(theta - (np.pi / 2)))
    for iter in np.arange(1,4+1).reshape(-1):
        __,E = elliptic12(invE,m,tol)
        invE = invE - (E - z) / np.sqrt(1 - np.multiply(m,np.sin(invE) ** 2))
