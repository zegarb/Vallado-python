...
st = 'y'
fid,outfile = open('fig712.out','wt')
for kt in np.arange(5,5+1).reshape(-1):
...
#      fprintf( kt:3,'fig 7-12 ro rtgt ',ro(4):11:7,rtgt(4):11:7 );
    ...
    if (kt > 0) and (kt < 5):
        vinto = np.array([0.0,np.sqrt(mu / mag(rinto)),0.0])
        vtgto = np.array([0.0,np.sqrt(mu / mag(rtgto)),0.0])
    if kt == 5:
    ...
    if kt == 7:
        ...
        vinto = np.array([2.604057,- 7.105717,- 0.263218])
        rtgto = np.array([1.0 * np.cos(trangle),1.0 * np.sin(trangle),0.0])
        vtgto = np.array([- 1.962372,7.323674,0.0])
        dt = 20 * tumin
        direc = 's'
        kepmov = 'n'
