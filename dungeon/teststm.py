...
outfile = open(strcat(directory,'teststm.out'),'wt')
mu = 398600.4418
...
for i in np.arange(1,10+1).reshape(-1):
    ...
    magv = mag(v)
    c1 = magv * magv - mu / mag(r)
    rdotv = np.dot(r,v)
    for ii in np.arange(1,3+1).reshape(-1):
...
vo = vorig
x = np.transpose(np.array([ro,vo]))
for i in np.arange(0,10+1).reshape(-1):
...
for i in np.arange(1,10+1).reshape(-1):
    ...
    f = 1.0 - 0.5 * u * dt ** 2 + 0.5 * u * p * dt ** 3 + 1.0 / 24.0 * (3 * u * q - 15 * u * p ** 2 + u ** 2) * dt ** 4 + 1.0 / 8.0 * (7 * u * p ** 3 - 3 * u * p * q - u ** 2 * p) * dt ** 5 + 1.0 / 720.0 * (630 * u * p ** 2 * q - 24 * u ** 2 * q - u ** 3 - 45 * u * q ** 2 - 945 * u * p ** 4 + 210 * u ** 2 * p ** 2) * dt ** 6 + 1.0 / 5040 * (882 * u ** 2 * p * q - 3150 * u ** 2 * p ** 3 - 9450 * u * p ** 3 * q + 1575 * u * p * q ** 2 + 63 * u ** 3 * p + 10395 * u * p ** 5) * dt ** 7 + 1.0 / 40320 * (1107 * u ** 2 * q ** 2 - 24570 * u ** 2 * p ** 2 * q - 2205 * u ** 3 * p ** 2 + 51975 * u ** 2 * p ** 4 - 42525 * u * p ** 2 * q ** 2 + 155925 * u * p ** 4 * q + 1575 * u * q ** 3 + 117 * u ** 3 * q - 135135 * u * p ** 6 + u ** 4) * dt ** 8
    g = dt - 1 / 6 * u * dt ** 3 + 0.25 * u * p * dt ** 4 + 1 / 120 * (9 * u * q - 45 * u * p ** 2 + u ** 2) * dt ** 5 + 1 / 360 * (210 * u * p ** 3 - 90 * u * p * q - 15 * u ** 2 * p) * dt ** 6 + 1 / 5040 * (3150 * u * p ** 2 * q - 54 * u ** 2 * q - 225 * u * q ** 2 - 4725 * u * p ** 4 + 630 * u ** 2 * p ** 2 - u ** 3) * dt ** 7 + 1 / 40320 * (3024 * u ** 2 * p * q - 12600 * u ** 2 * p ** 3 - 56700 * u * p ** 3 * q + 9450 * u * p * q ** 2 + 62370 * u * p ** 5 + 126 * u ** 3 * p) * dt ** 8
    fdot = - u * dt + 1.5 * u * p * dt ** 2 + 1.0 / 6.0 * (3 * u * q - 15 * u * p ** 2 + u ** 2) * dt ** 3 + 5.0 / 8.0 * (7 * u * p ** 3 - 3 * u * p * q - u ** 2 * p) * dt ** 4 + 6.0 / 720.0 * (630 * u * p ** 2 * q - 24 * u ** 2 * q - u ** 3 - 45 * u * q ** 2 - 945 * u * p ** 4 + 210 * u ** 2 * p ** 2) * dt ** 5 + 7.0 / 5040 * (882 * u ** 2 * p * q - 3150 * u ** 2 * p ** 3 - 9450 * u * p ** 3 * q + 1575 * u * p * q ** 2 + 63 * u ** 3 * p + 10395 * u * p ** 5) * dt ** 6 + 8.0 / 40320 * (1107 * u ** 2 * q ** 2 - 24570 * u ** 2 * p ** 2 * q - 2205 * u ** 3 * p ** 2 + 51975 * u ** 2 * p ** 4 - 42525 * u * p ** 2 * q ** 2 + 155925 * u * p ** 4 * q + 1575 * u * q ** 3 + 117 * u ** 3 * q - 135135 * u * p ** 6 + u ** 4) * dt ** 7
    gdot = 1 - 0.5 * u * dt ** 2 + u * p * dt ** 3 + 5 / 120 * (9 * u * q - 45 * u * p ** 2 + u ** 2) * dt ** 4 + 1 / 60 * (210 * u * p ** 3 - 90 * u * p * q - 15 * u ** 2 * p) * dt ** 5 + 7 / 5040 * (3150 * u * p ** 2 * q - 54 * u ** 2 * q - 225 * u * q ** 2 - 4725 * u * p ** 4 + 630 * u ** 2 * p ** 2 - u ** 3) * dt ** 6 + 8 / 40320 * (3024 * u ** 2 * p * q - 12600 * u ** 2 * p ** 3 - 56700 * u * p ** 3 * q + 9450 * u * p * q ** 2 + 62370 * u * p ** 5 + 126 * u ** 3 * p) * dt ** 7
    stm[1,1] = f
    stm[2,2] = f
    stm[3,3] = f
    stm[1,4] = g
...
for i in np.arange(0,36+1).reshape(-1):
    lon = i * 10 * deg2rad
    for j in np.arange(- 9,9+1).reshape(-1):
        lat = j * 10 * deg2rad
        # form position vectors
        r1 = ro
        r2 = magr * np.array([np.cos(lat) * np.cos(lon),np.cos(lat) * np.sin(lon),np.sin(lat)])
        # loop through these
        dtsec = 7635.0
        ...
        partr2wpartv1 = magr1 / mu * (1.0 - f) * (np.transpose((r2 - r1)) * v1 - np.transpose((v2 - v1)) * r1) + cb / mu * np.transpose(v2) * v1 + g * np.eye(3)
        #partr2wpartv1;
        det(partr2wpartv1)
