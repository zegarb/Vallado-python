...
print('output directory set to: %s, change in testmat.m if needed \n' % (directory))
# global opsmode
...
infilename = "put a filename here"
...
if (typerun == 'c'):
    outfile = open(os.path.join(directory,'tmatall.out'),'wt')
else:
    if (typerun == 'v'):
        outfile = open(os.path.join(directory,'tmatver.out'),'wt')
    else:
        outfile = open(os.path.join(directory,'tmat.out'),'wt')
...
while (not feof(infile) ):
    ...
    if (feof(infile) == 0):
        ...
        if idebug:
            catno = longstr1[2:7]
            dbgfile = open(os.path.join('sgp4test.dbg.',catno),'wt')
...
stopmfe = 2880.0
deltamin = 120.0
satrec = sgp4init(whichconst,opsmode,satrec,satrec.jdsatepoch + satrec.jdsatepochf - 2433281.5,satrec.bstar,satrec.ndot,satrec.nddot,satrec.ecco,satrec.argpo,satrec.inclo,satrec.mo,satrec.no_kozai,satrec.nodeo)
tsince = startmfe
while ((tsince < stopmfe) and (satrec.error == 0)):
