...
def ap2kp(apin=None):
    ap = np.array([- 1e-05, - 0.001, 0, 2, 3, 4, 5, 6, 7, 9, 12, 15, 18, 22, 27, 32, 39, 48,
                   56, 67, 80, 94, 111, 132, 154, 179, 207, 236, 300, 400, 900])
    kp = np.array([- 0.66666667, - 0.33333, 0.0, 0.33333, 0.66667, 1.0, 1.33333,
                   1.66667, 2.0, 2.33333, 2.66667, 3.0, 3.33333, 3.66667, 4.0, 4.33333,
                   4.66667, 5.0, 5.33333, 5.66667, 6.0, 6.33333, 6.66667, 7.0, 7.33333,
                   7.66667, 8.0, 8.33333, 8.66667, 9.0, 9.33333])
    kpout = 0.0
    ...
    if (i > 2):
        ...
        p3 = kp[i]
        p4 = kp[i + 1]
        ac0, ac1, ac2, ac3 = smu.cubicspl(p1, p2, p3, p4) #t1, t2
        p1 = ap[i - 2]
        p2 = ap[i - 1]
        p3 = ap[i]
        p4 = ap[i + 1]
        kac0, kac1, kac2, kac3 = smu.cubicspl(p1, p2, p3, p4) #t1, t2
        #p1
        #p2
        #p3
        #p4
        ...
        if ((r[0] >= 0.0) and (r[0] <= 1.001)):
            ...
            if ((r[1] >= 0.0) and (r[1] <= 1.001)):
                apval = r[1]
            else:
                if ((r[2] >= 0.0) and (r[2] <= 1.001)):
                    apval = r[2]
                else:
                    apval = 0.0
